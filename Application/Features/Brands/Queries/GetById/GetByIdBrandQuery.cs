...
public class GetByIdBrandQuery : IRequest<GetByIdBrandResponse>
{
    ...
    public class GetByIdBrandQueryHandler : IRequestHandler<GetByIdBrandQuery, GetByIdBrandResponse>
    {
        ...
        {
           Brand? brands = await _brandRepository.GetAsync(predicate: b => b.Id == request.Id,withDeleted:true  , cancellationToken: cancellationToken);

            GetByIdBrandResponse response = _mapper.Map<GetByIdBrandResponse>(brands);
            return response;
        }
    }
